<?php

namespace App\Http\Controllers;

use App\Models\Admin; // Référence au modèle Admin
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\Auth;

class AdminController extends Controller // Nom de la classe changé ici
{
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required|string|size:36|exists:users,id|unique:administrateurs,id', // Le nom de la table reste 'administrateurs'
            'nom' => 'required|string|max:255',
            'prenom' => 'required|string|max:255',
            'service' => 'nullable|string|max:255',
        ]);

        if ($validator->fails()) {
            return response()->json(['errors' => $validator->errors()], 422);
        }

        $admin = new Admin(); // Utilisation du modèle Admin
        $admin->id = $validator->validated()['user_id'];
        $admin->fill($validator->validated());

        if (Auth::check()) {
             $admin->created_by_user_id = Auth::id();
        }

        $admin->save();

        return response()->json(['message' => 'Administrateur créé avec succès.', 'data' => $admin], 201);
    }

    public function update(Request $request, string $id)
    {
        $validator = Validator::make($request->all(), [
            'nom' => 'string|max:255',
            'prenom' => 'string|max:255',
            'service' => 'nullable|string|max:255',
        ]);

        if ($validator->fails()) {
            return response()->json(['errors' => $validator->errors()], 422);
        }

        $admin = Admin::find($id); // Utilisation du modèle Admin

        if (!$admin) {
            return response()->json(['message' => 'Administrateur non trouvé.'], 404);
        }

        $admin->fill($validator->validated());

         if (Auth::check()) {
            $admin->updated_by_user_id = Auth::id();
         }

        $admin->save();

        return response()->json(['message' => 'Administrateur mis à jour avec succès.', 'data' => $admin], 200);
    }

    public function destroy(string $id)
    {
        $admin = Admin::find($id); // Utilisation du modèle Admin

        if (!$admin) {
            return response()->json(['message' => 'Administrateur non trouvé.'], 404);
        }

        try {
            $admin->delete();
             return response()->json(['message' => 'Administrateur supprimé avec succès.', 'id' => $id], 200);
        } catch (\Exception $e) {
             return response()->json(['message' => 'Erreur lors de la suppression de l\'administrateur.', 'error' => $e->getMessage()], 500);
        }
    }
}
